...
vector<TXN> createTransactions(vector<Node> Peers, int n_peers, double mean_interarrival_time, int num_transactions) {
    vector<TXN> cur_txn;
    ...
    for (int i = 0; i < num_transactions; i++) {
        ...
        int sender_id = generateRandom(1, n_peers); 
        int receiver_id = generateRandom(1, n_peers);

        // Ensure sender and receiver are different
        while (receiver_id == sender_id) {
            receiver_id = generateRandom(1, n_peers);
        }
        int transaction_amount = generateRandom(1, 20);
        // Check if the sender has enough money to perform the transaction
        if (Peers.getBalance(sender_id) >= transaction_amount) {
            TXN txn;
            ...
            Peers.updateBalance(Peers, sender_id, -txn.amount, n_peers);
            Peers.updateBalance(Peers, receiver_id, txn.amount, n_peers);
            
            cur_txn.push_back(txn);

            cout << "Time: " << current_time << " TxnID: " << txn.txn_id << " " << txn.sender_id << " pays " << txn.receiver_id << " " << txn.amount << " coins" << endl;
        }
    }
